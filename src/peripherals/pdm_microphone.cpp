...
// The pdm_microphone.pio needs to be compiled with PIOASM to *.pio.h
// Pre-compiled PIOASM can be found in Arduino packages/rp2040/tools/pqt-pioasm
...
int pdm_microphone_init(const struct pdm_microphone_config* config) {
    ...
    for (int i = 0; i < PDM_RAW_BUFFER_COUNT; i++) {
        ...
        pdm_mic.raw_buffer[i] = new uint8_t(pdm_mic.raw_buffer_size);
...
int pdm_microphone_read(int16_t* buffer, size_t samples) {
    ...
    for (size_t i = 0; i < samples; i += filter_stride) {
